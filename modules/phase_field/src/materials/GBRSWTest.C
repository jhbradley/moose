...
InputParameters validParams<GBRSWTest>()
{
  ...
  params.addParam<Real>("GBmob0", 9.2124e-6, "Grain boundary mobility prefactor in m^4/(J*s)");
  params.addParam<Real>("Q", .277, "Grain boundary migration activation energy in eV");
  params.addParam<Real>("GBenergy", 1.58, "Grain boundary energy in J/m^2");
...
GBRSWTest::GBRSWTest(const std::string & name,
...
// This is the test to use the gradient of the order parameter to pick a gb energy
  ...
  _grad_size = std::sqrt((*_grad_v[0])[_qp]*(*_grad_v[0])[_qp]);
  _mag_grad_comp = std::sqrt((*_grad_v[0])[_qp](0)*(*_grad_v[0])[_qp](0));
  ...
  if (_mag_grad_comp/_grad_size <= 10e-10)
    mooseError("taking log of zero due to rounding!");

  _GBenergy = 1 + (2-1) * _mag_grad_comp/_grad_size * (1 - log(_mag_grad_comp/_grad_size));
  if (libmesh_isnan(_GBenergy))
    mooseError("The Energy diverges");
