...
[Materials]
  [./CuGrGr]
    # Material properties
    type = GBRSWTest # Quantitative material properties for copper grain growth.  Dimensions are nm and ns
    block = 0 # Block ID (only one block in this problem)
    GBmob0 = 9.2124e-6 #Mobility prefactor for Cu from Schonfelder1997
    GBenergy = 1.58 #GB energy for Cu from Schonfelder1997
    Q = .277 #Activation energy for grain growth from Schonfelder 1997
    T = 650 # K   #Constant temperature of the simulation (for mobility calculation)
  ...
  [./grain0]
    ...
    block = 0
    args = 'gr0'
    function = 'gr0/gr0-gr0' #I am worried that this will fail here.
    outputs = exodus
  [../]
  [./grain1]
    type = ParsedMaterial
